  ...
  fetch: withSupabase(
    ...
    async (req, ctx) => {
      try {
        ...
        const pedidoId =
          body?.pedido_id?.toString();

        const editorId =
          body?.editor_id?.toString();
        ...
        if (!editorId) {
          throw new Error(
            "No se recibió editor_id.",
          );
        }

        const { data: editor, error: errorEditor } =
        ...
        if (errorEditor || !editor) {
          throw new Error(
            "No se pudo identificar al usuario que editó el pedido.",
          );
        }

        if (editor.rol?.toString() === "administrador") {
          return Response.json({
            ok: true,
            enviados: 0,
            motivo: "La edición fue realizada por un administrador.",
          });
        }
        ...
        const nombreEditor = [
          editor.nombre?.toString().trim(),
          editor.apellido?.toString().trim(),
        ]
          .filter(Boolean)
          .join(" ");
        ...
        const { data: admins, error: errorAdmins } =
          await ctx.supabaseAdmin
            .from("usuarios")
            .select("id")
        ...
        if (errorAdmins) {
          throw new Error(
            `No se pudieron consultar administradores: ${errorAdmins.message}`,
          );
        }
        ...
        if (
          ...
          throw new Error(
            "No hay dispositivos Android activos de administradores.",
          );
        }
              ...
              body: JSON.stringify({
                message: {
                  ...
                  notification: {
                    ...
                    body:
                      `${nombreEditor || "Un preventista"} modificó el pedido de ${nombreCliente}.\n` +
                      `Nuevo total: ${totalTexto}`,
                  },
                  data: {
                    tipo: "pedido_editado",
                    pedido_id: pedidoId,
